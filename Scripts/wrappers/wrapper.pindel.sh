...
echo "***************************************************"
echo "NOTE THIS WILL ONLY WORK ON RUSSELL'S ACCOUNT BECAUSE OF BIOCONDA"
echo ""
...
. /u/home/r/rlittman/miniconda3/etc/profile.d/conda.sh
...
echo "samtools view $input | sam2pindel - ${filename}.pindeloutput.txt 300 tomour 0 >> $logfile 2>&1" >> $logfile

samtools view $input | sam2pindel - ${filename}.pindeloutput.txt 300 tomour 0 >> $logfile 2>&1

echo "pindel -f $reference -p ${filename}.pindeloutput.txt -c ALL -o $outdir/${filename} >> $logfile 2>&1" >> $logfile

pindel -f $reference -p ${filename}.pindeloutput.txt -c ALL -o $outdir/${filename} >> $logfile 2>&1
